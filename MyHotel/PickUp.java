...
public class PickUp extends JFrame {
	...
	public PickUp() throws SQLException {
		...
		JLabel lblPickUpService = new JLabel("Serviço de Valet");
		...
		JLabel lblTypeOfCar = new JLabel("Tipo de carro");
		...
		contentPane.add(lblTypeOfCar);
                ...
                try{
                    conn c = new conn();
                    ResultSet rs = c.s.executeQuery("SELECT * FROM MOTORISTAS");
                    while(rs.next()){
                        c1.add(rs.getString("marca"));    
                    }
                }catch(Exception e){ }
		...
		JLabel lblInfo = new JLabel("Nome");
		lblInfo.setBounds(24, 208, 46, 14);
		contentPane.add(lblInfo);
		
		JButton btnRegister = new JButton("Mostrar");
		btnRegister.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String SQL = "SELECT * FROM MOTORISTAS where marca = '"+c1.getSelectedItem()+"'";
				try{
