...
public class ManagerInfo extends JFrame {
	...
	public ManagerInfo() throws SQLException {
		...
		JButton btnLoadData = new JButton("Carregar Dados");
		btnLoadData.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try{
                                    conn c = new conn();
				String displayCustomersql = "SELECT * FROM FUNCIONARIOS where trabalho = 'Gerente'";
		...
		lblJob = new JLabel("Idade");
		lblJob.setBounds(159, 11, 46, 14);
		contentPane.add(lblJob);
		
		lblName = new JLabel("Gênero");
		lblName.setBounds(273, 11, 46, 14);
		contentPane.add(lblName);
		
		lblDepartment = new JLabel("Trabalho");
		lblDepartment.setBounds(416, 11, 86, 14);
		contentPane.add(lblDepartment);
                
                JLabel l1 = new JLabel("Salário");
