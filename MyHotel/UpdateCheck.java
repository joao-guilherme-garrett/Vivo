...
public class UpdateCheck extends JFrame {
	...
	public UpdateCheck() throws SQLException {
		...
		JLabel lblUpdateCheckStatus = new JLabel("Detalhes do Check-In");
		...
		contentPane.add(lblNewLabel);
                ...
                try{
                    conn c = new conn();
                    ResultSet rs = c.s.executeQuery("SELECT * FROM CLIENTES");
                    while(rs.next()){
                        c1.add(rs.getString("numero"));    
                    }
                }catch(Exception e){ }
		...
		JLabel lblNewLabel_3 = new JLabel("Check-in em :");
		lblNewLabel_3.setBounds(25, 216, 107, 14);
		contentPane.add(lblNewLabel_3);
		
		JLabel lblNewLabel_4 = new JLabel("Quantia paga (R$) : ");
		lblNewLabel_4.setBounds(25, 261, 107, 14);
		contentPane.add(lblNewLabel_4);
		
		JLabel lblNewLabel_5 = new JLabel("Quantia pendente (R$) : ");
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) { 
                            try{
                                ...
                                c.s.executeUpdate("UPDATE CLIENTES SET numero_quarto = '"+s2+"', nome = '"+s3+"', status = '"+s4+"', deposito = '"+s5+"' WHERE numero = '"+s1+"'");
		...
		JButton btnAdd = new JButton("Entrar/Sair");
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
                            try{
				String s1 = c1.getSelectedItem();
				conn c = new conn();
                                ResultSet rs1 = c.s.executeQuery("SELECT * FROM CLIENTES WHERE numero = "+s1);
                                
                                while(rs1.next()){
                                    txt_ID.setText(rs1.getString("numero_quarto"));    
                                    txt_Status.setText(rs1.getString("nome"));    
                                    txt_Date.setText(rs1.getString("status"));    
                                    txt_Time.setText(rs1.getString("deposito"));    
                                }
                            }catch(Exception ee){}
                            ...
                            try{
                                String total = "";
                                conn c  = new conn();
                                ResultSet rs2 = c.s.executeQuery("SELECT * FROM QUARTOS WHERE numero_quarto = "+txt_ID.getText());
                                while(rs2.next()){
                                    total = rs2.getString("preço"); 
